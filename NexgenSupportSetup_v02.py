...
def rename_tailscale_adapter():
    ...
    try:
        ...
        if OLD_NAME in adapters:
            print(f"[INFO] Renaming '{OLD_NAME}' to '{NEW_NAME}'...")
            subprocess.run(
                ["powershell", "-Command",
                 f"Rename-NetAdapter -Name '{OLD_NAME}' -NewName '{NEW_NAME}'"],
                check=True
            )
            print("[SUCCESS] Adapter renamed.")
        else:
            print("[INFO] Tailscale adapter not found.")

    except Exception as e:
        print(f"[ERROR] Failed to rename adapter: {e}")
